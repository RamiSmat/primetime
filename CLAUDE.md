...
Currently implemented: a `primetime prime <provider>` CLI backed by a real Codex adapter that runs one minimal, sandboxed `codex exec` request through the official Codex CLI (see the Codex provider section in the README for the exact safety flags and failure classification). `CodexProvider.setup()` transfers your local Codex session (`$CODEX_HOME/auth.json`) directly to the current repository's `CODEX_AUTH_JSON` GitHub Actions secret via `gh secret set` (see `@primetime/github`) — it does not authenticate Codex itself; run `codex login` locally first. `primetime schedule next <config-path>` reads a schedule config file and prints the next primer run, computed by the timezone/DST-aware `@primetime/scheduler` package. Neither `schedule next` nor `setup()` are wired into an actual scheduled GitHub Actions workflow yet — that piece (bootstrap `auth.json` from the secret → run `prime` → write the refreshed `auth.json` back) is not yet built; see the README's "GitHub Actions credential setup" section for the constraints it will need to respect. The web UI is not yet built.
...
- `templates/github-actions` — will hold the actual scheduled workflow (bootstrap `auth.json` from the secret, run `prime`, write the refreshed `auth.json` back) once built; workflows must use only user-controlled secrets and never embed credentials. See the README's "GitHub Actions credential setup" section for the `auth.json` refresh/write-back constraints this depends on, sourced from OpenAI's own CI/CD auth docs — don't design this from assumptions without re-reading that section.

Package import boundaries: `apps/cli` depends on `@primetime/providers`, `@primetime/scheduler`, and `@primetime/shared`; `@primetime/providers` depends on `@primetime/github` and `@primetime/shared`; `@primetime/github` and `@primetime/scheduler` each depend on `@primetime/shared`. Keep provider-specific logic out of `shared` and out of the CLI layer — new provider behavior belongs in its own subdirectory under `packages/providers/src/`.
